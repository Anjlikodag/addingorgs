...
In this sample it is assumed that only one organization (played by Apple) is in a central banker role and can mint new tokens into their account, while any organization can transfer tokens from their account to a recipient's account.
...
The minter intends to transfer 100 tokens to the Fiserv recipient, but first the Fiserv recipient needs to provide their own account ID as the payment address.
...
Using the Fiserv terminal, the Fiserv recipient user can retrieve their own account ID:
```
...
Let's base64 decode the account ID to make sure it represents the Fiserv recipient user:
```
...
The result shows that the subject and issuer is indeed the recipient user from Fiserv:
```
x509::/C=US/ST=North Carolina/O=Hyperledger/OU=client/CN=recipient::/C=UK/ST=Hampshire/L=Hursley/O=fiserv.example.com/CN=ca.fiserv.example.com
```

After the Fiserv recipient provides their account ID to the minter, the minter can initiate a transfer from their account to the recipient's account.
Back in the Apple terminal, request the transfer of 100 tokens to the recipient account:
...
**For a JavaScript Contract:**
```
export RECIPIENT="x509::/C=US/ST=North Carolina/O=Hyperledger/OU=client/CN=recipient::/C=UK/ST=Hampshire/L=Hursley/O=fiserv.example.com/CN=ca.fiserv.example.com"
peer chaincode invoke $TARGET_TLS_OPTIONS -C mychannel -n token_erc20 -c '{"function":"Transfer","Args":[ "'"$RECIPIENT"'","100"]}'
```
...
While still in the Apple terminal, let's request the minter's account balance again:
```
peer chaincode query -C mychannel -n token_erc20 -c '{"function":"ClientAccountBalance","Args":[]}'
```
...
And then using the Fiserv terminal, let's request the recipient's balance:
```
peer chaincode query -C mychannel -n token_erc20 -c '{"function":"ClientAccountBalance","Args":[]}'
```
...
We will use the Apple CA to create the spender identity.
Back in the Apple terminal, you can register a new spender client identity using the `fabric-ca-client` tool:
```
fabric-ca-client register --caname ca-apple --id.name spender --id.secret spenderpw --id.type client --tls.certfiles ${PWD}/organizations/fabric-ca/apple/tls-cert.pem
```
...
```
fabric-ca-client enroll -u https://spender:spenderpw@localhost:7054 --caname ca-apple -M ${PWD}/organizations/peerOrganizations/apple.example.com/users/spender@apple.example.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/apple/tls-cert.pem
```

Run the command below to copy the Node OU configuration file into the spender identity MSP folder.
```
cp ${PWD}/organizations/peerOrganizations/apple.example.com/msp/config.yaml ${PWD}/organizations/peerOrganizations/apple.example.com/users/spender@apple.example.com/msp/config.yaml
```
...
Open a 3rd terminal to represent the spender in Apple and navigate to fabric-samples/test-network. Set the the environment variables for the Apple spender user.
...
export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID="AppleMSP"
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/apple.example.com/users/spender@apple.example.com/msp
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/apple.example.com/peers/peer0.apple.example.com/tls/ca.crt
export CORE_PEER_ADDRESS=localhost:7051
export TARGET_TLS_OPTIONS="-o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile ${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem --peerAddresses localhost:7051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/apple.example.com/peers/peer0.apple.example.com/tls/ca.crt --peerAddresses localhost:9051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/fiserv.example.com/peers/peer0.fiserv.example.com/tls/ca.crt"
```

Now the Apple spender can retrieve their own client ID:
```
peer chaincode query -C mychannel -n token_erc20 -c '{"function":"ClientAccountID","Args":[]}'
```
...
The result shows that the subject and issuer is indeed the recipient user from Fiserv:
```
x509::/C=US/ST=North Carolina/O=Hyperledger/OU=client/CN=spender::/C=US/ST=North Carolina/L=Durham/O=apple.example.com/CN=ca.apple.example.com
```

After the Apple spender provides their client ID to the minter, the minter can approve a spender.
Back in the Apple minter terminal, request the approval of 500 tokens to be withdrawn by the spender.
...
```
export SPENDER="x509::/C=US/ST=North Carolina/O=Hyperledger/OU=client/CN=spender::/C=US/ST=North Carolina/L=Durham/O=apple.example.com/CN=ca.apple.example.com"
peer chaincode invoke $TARGET_TLS_OPTIONS -C mychannel -n token_erc20 -c '{"function":"Approve","Args":["'"$SPENDER"'", "500"]}'
```
...
Let's request the spender's allowance from the Apple minter terminal.
...
```
export MINTER="x509::/C=US/ST=North Carolina/O=Hyperledger/OU=client/CN=minter::/C=US/ST=North Carolina/L=Durham/O=apple.example.com/CN=ca.apple.example.com"
peer chaincode query -C mychannel -n token_erc20 -c '{"function":"Allowance","Args":["'"$MINTER"'", "'"$SPENDER"'"]}'
```
...
The spender intends to transfer 100 tokens to the Fiserv recipient on behalf of the minter. The spender has already got the minter client Id and the recipient client ID.
...
* You could also require that accounts get setup before use, and apply state-based endorsement for each account key that has been created. For example on an Apple account, set state-based endorsement policy to be Apple and the central banker (or some other trust anchor). And on an Fiserv account, set state-based endorsement policy to be Fiserv and the central banker (or some other trust anchor). Then to transfer tokens from an Apple account to an Fiserv account, you would require endorsements from Apple, Fiserv, and the central banker (or some other trust anchor).
