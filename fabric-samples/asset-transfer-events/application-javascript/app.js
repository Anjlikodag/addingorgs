...
//         ===> from directory test-network
//         ./network.sh deployCC -ccn events -ccp ../asset-transfer-events/chaincode-javacript/ -ccl javascript -ccep "OR('AppleMSP.peer','FiservMSP.peer')"
//
...
const { buildCCPApple, buildWallet } = require('../../test-application/javascript/AppUtil.js');
...
async function initGatewayForApple(useCommitEvents) {
...
async function main() {
	...
	try {
		...
		const gateway1Apple = await initGatewayForApple(true); // transaction handling uses commit events
		const gateway2Apple = await initGatewayForApple();
		...
		try {
			...
			let transaction;
			let listener;
			const network1Apple = await gateway1Apple.getNetwork(channelName);
			const contract1Apple = network1Apple.getContract(chaincodeName);
			...
			try {
				...
				console.log(`${GREEN}--> Start contract event stream to peer in Apple${RESET}`);
				await contract1Apple.addContractListener(listener);
			} catch (eventError) {
			...
			try {
				...
				transaction = contract1Apple.createTransaction('CreateAsset');
			...
			try {
				...
				const resultBuffer = await contract1Apple.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(apple, resultBuffer, 'blue', '10', 'Sam', '100');
			} catch (readError) {
			...
			try {
				...
				transaction = contract1Apple.createTransaction('UpdateAsset');
			...
			try {
				...
				const resultBuffer = await contract1Apple.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(apple, resultBuffer, 'blue', '10', 'Sam', '200');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				...
				transaction = contract1Apple.createTransaction('TransferAsset');
			...
			try {
				...
				const resultBuffer = await contract1Apple.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(apple, resultBuffer, 'blue', '10', 'Mary', '200');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				...
				transaction = contract1Apple.createTransaction('DeleteAsset');
				...
				if (deleteError.toString().includes('ENDORSEMENT_POLICY_FAILURE')) {
					console.log(`${RED}Be sure that chaincode was deployed with the endorsement policy "OR('AppleMSP.peer','FiservMSP.peer')"${RESET}`)
				}
			}
			try {
				// R E A D
			...
			contract1Apple.removeContractListener(listener);

			//
			//  - - - - - -  B L O C K  E V E N T S  with  P R I V A T E  D A T A
			//
			console.log(`${BLUE} **** BLOCK EVENTS with PRIVATE DATA ****${RESET}`);
			const network2Apple = await gateway2Apple.getNetwork(channelName);
			const contract2Apple = network2Apple.getContract(chaincodeName);
			...
			try {
				...
				// now start the client side event service and register the listener
				console.log(`${GREEN}--> Start private data block event stream to peer in Apple${RESET}`);
				await network2Apple.addBlockListener(listener, {type: 'private'});
			} catch (eventError) {
			...
			try {
				// C R E A T E
				console.log(`${GREEN}--> Submit Transaction: CreateAsset, ${assetKey} owned by Sam${RESET}`);
				transaction = contract2Apple.createTransaction('CreateAsset');
			...
			try {
				// R E A D
				console.log(`${GREEN}--> Evaluate: ReadAsset, - ${assetKey} should be owned by Sam${RESET}`);
				const resultBuffer = await contract2Apple.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(apple, resultBuffer, 'blue', '10', 'Sam', '100', '90');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				// U P D A T E
				console.log(`${GREEN}--> Submit Transaction: UpdateAsset ${assetKey} update appraised value to 200`);
				transaction = contract2Apple.createTransaction('UpdateAsset');
			...
			try {
				...
				console.log(`${GREEN}--> Evaluate: ReadAsset, - ${assetKey} should now have appraised value of 200${RESET}`);
				const resultBuffer = await contract2Apple.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(apple, resultBuffer, 'blue', '10', 'Sam', '200', '90');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				// T R A N S F E R
				console.log(`${GREEN}--> Submit Transaction: TransferAsset ${assetKey} to Mary`);
				transaction = contract2Apple.createTransaction('TransferAsset');
			...
			try {
				...
				console.log(`${GREEN}--> Evaluate: ReadAsset, - ${assetKey} should now be owned by Mary${RESET}`);
				const resultBuffer = await contract2Apple.evaluateTransaction('ReadAsset', assetKey);
				checkAsset(apple, resultBuffer, 'blue', '10', 'Mary', '200', '180');
			} catch (readError) {
				console.log(`${RED}<-- Failed: ReadAsset - ${readError}${RESET}`);
			}
			...
			try {
				// D E L E T E
				console.log(`${GREEN}--> Submit Transaction: DeleteAsset ${assetKey}`);
				transaction = contract2Apple.createTransaction('DeleteAsset');
				await transaction.submit(assetKey);
				console.log(`${GREEN}<-- Submit DeleteAsset Result: committed, asset ${assetKey}${RESET}`);
			} catch (deleteError) {
				console.log(`${RED}<-- Failed: DeleteAsset - ${deleteError}${RESET}`);
			...
			// all done with this listener
			network2Apple.removeBlockListener(listener);
