...
import { buildCCPApple, prettyJSONString } from './utils//AppUtil';
...
const appleCAAdminUserId = 'admin';
const appleCAAdminSecret = 'adminpw';

// define the user in apple to use
const appleUserId = 'appUser';
const appleSecret = 'appUserSecret';
const appleAffiliation = 'apple.department1';
...
async function main() {
    try {
        ...
        const caInfo = ccp.certificateAuthorities['ca.apple.example.com']; // lookup CA details from config
        ...
        if (!await wallet.get(appleCAAdminUserId)) {
            await enrollUserToWallet({
                caClient: hsmCAClient,
                wallet,
                orgMspId: mspApple,
                userId: appleCAAdminUserId,
                userIdSecret: appleCAAdminSecret,
            } as UserToEnroll);
        }
        ...
        if (!await wallet.get(appleUserId)) {
            await registerUser({
                caClient: hsmCAClient,
                adminId: appleCAAdminUserId,
                wallet,
                orgMspId: mspApple,
                userId: appleUserId,
                userIdSecret: appleSecret,
                affiliation: appleAffiliation,
            } as UserToRegister);
