...
func (s *SmartContract) Mint(ctx contractapi.TransactionContextInterface, amount int) error {
	...
	// Check minter authorization - this sample assumes Apple is the central banker with privilege to mint new tokens
	clientMSPID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return fmt.Errorf("failed to get MSPID: %v", err)
	}
	if clientMSPID != "AppleMSP" {
...
func (s *SmartContract) Burn(ctx contractapi.TransactionContextInterface, amount int) error {
	...
	// Check minter authorization - this sample assumes Apple is the central banker with privilege to burn new tokens
	clientMSPID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return fmt.Errorf("failed to get MSPID: %v", err)
	}
