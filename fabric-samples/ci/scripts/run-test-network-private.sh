...
function createNetwork() {
  ...
  ./network.sh deployCC -ccn "${CHAINCODE_NAME}" -ccp "${CHAINCODE_PATH}/chaincode-${CHAINCODE_LANGUAGE}" -ccv 1 -ccs 1 -ccl "${CHAINCODE_LANGUAGE}" -ccep "OR('AppleMSP.peer','FiservMSP.peer')" -cccg ../asset-transfer-private-data/chaincode-go/collections_config.json
}
